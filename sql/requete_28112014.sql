...
create or replace function itineraireb(A integer, B integer, C float)
  returns table (param_seq integer, param_node integer, param_edge integer, param_cost float, param_coef float, param_geom text)
as
$body$
	SELECT seq, id1 as node, id2 as edge,  cost , coeff_eclairage, st_asgeojson(geom) FROM pgr_dijkstra('
		SELECT gid AS id,
...
create or replace function itib(X1 float, Y1 float,X2 float, Y2 float, C float)
  returns table (param_seq integer, param_node integer, param_edge integer, param_cost float, param_coef float, param_geom text)
as
$body$
	--with n1 as (SELECT * FROM point($1,$2)), n2 as (SELECT * FROM point($3,$4))
	SELECT * FROM itineraireb(pointD($1,$2),pointD($3,$4),$5)
$body$
language sql;
...
-- Test 
SELECT *
FROM itib(14.66439117,-17.44131088,14.70358007,-17.45264053,0)
